...
const objectsEqual = (obj1, obj2) => {
  ...
  if (obj1Arr.length != obj2Arr.length) {
    output = false;
    return output; //we can just return false without messing without output
  }
  // second case: the objects keys don't have the same values
  for (const prop in obj1) {
    if (obj1[prop] != obj2[prop]) {
      output = false;
      return output; // nice logic flow just same comment as above ^^
    }
  }
  ...
  for (let i = 0; i < obj1Arr.length; i++) {
    if (!obj2Arr.includes(obj1Arr[i])) {
      output = false;
      return output; // nice yep looking for falsey
    }
  }
  return output;
};
