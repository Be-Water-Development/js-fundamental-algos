...
// good stuff - map returns a new array always
...
// nice you went above and beyond here-- filter natively can't handle an object so this is overkill, only an array is necessary
// great logic
...
function reject(collection, callback) {
  if (Array.isArray(collection)) {
    ...
    // ->> code smell just an if and then the rest of the code, far fewer brackets, avoid else ifs same as else usually
...
function indexOf(array, value) {
  ...
  return array.reduce((acc, val, ind) => {
    if ((val === value) & (counter === 0)) {
      // a single & is not a thing; something wierd is going on here take another look at this one
      acc = ind;
      counter++;
    }
    return acc;
...
function every(array, func) {
  ...
  let boolArr = array.reduce((acc, val) => {
    ...
    return acc;
  }, []); // congested code, work on style smell here
...
// check out this one again if you have the time/energy -- this is not recursive, recusive is to call itself
